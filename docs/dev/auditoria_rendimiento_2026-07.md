...
### Fase 3 — Fondo y pulido · **HECHA (commits `9bc9361` + `0fca2bf`)**: N8 (worker SQL singleton + lazy-mount por visibilidad con `LazyVisible`), N9 (caché DESCRIBE LRU + Promise.all), G10 (lazy imports: chunk principal 3.593→2.064 kB, −43%), G6 (drags con mutación directa de DOM + commit único), G7 (memo TabBar/WindowTitleBar/CommandPalette + handlers estables; AiAssistantPanel sin memo — pulido futuro), G11 (66 `transition: all` → propiedades explícitas; App.css eliminado), E5 (conda exec async, xlsx readFile async, notebook-state/config writeFile async; fs síncronos de rutas frías aceptados; xlsx a worker_thread pendiente), E8 (historial en batch 3s/20 con flush en close y en el endpoint de lectura). **Pulido futuro opcional**: `manualChunks` de vite, typewriter adaptativo, memo de AiAssistantPanel, xlsx en worker_thread.
