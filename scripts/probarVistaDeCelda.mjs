...
// ── cambiar de tipo NO se decide aquí ───────────────────────────────────────
// `CREATE OR REPLACE` no sabe cambiar el tipo de un objeto, así que al pulsar
// «Materializar» hay que relevar lo que había. Pero eso exige mirar el catálogo
// —`DROP ... IF EXISTS` falla si el nombre existe con OTRO tipo, no sólo si no
// existe—, y el catálogo es del motor. Lo hace el servidor al ejecutar; la
// secuencia entera se prueba en `probarRelevoDeTipo.mjs`, contra DuckDB.
//
// Aquí lo único que se comprueba es que de aquí NO sale ningún `DROP`: mandarlo
// a ciegas es justo lo que rompía volver a ejecutar una celda.
{
    const vista = componer('SELECT 1', { nombre: 'p' }).preparacion;
    const tabla = componer('SELECT 1', { nombre: 'p', materializar: true }).preparacion;
    comprobar('una celda normal no manda ningún DROP', /\bDROP\b/i.test(vista), false);
    comprobar('ni una materializada', /\bDROP\b/i.test(tabla), false);
}
