...
/**
 * INLINE_FIELDS — los campos que se pueden editar DENTRO del nodo al expandirlo
 * (fase 7 de docs/dev/plan_rediseno_visual.md).
 *
 * Deliberadamente NO es toda la configuración. Solo entran campos de TEXTO LIBRE
 * —rutas, nombres de tabla, claves, consultas— por dos razones:
 *
 *   1. Los campos con opciones cerradas (format, joinType, aggFunc...) exigirían
 *      replicar aquí sus listas de valores, y una lista que se desincronice del
 *      ejecutor produce configuraciones que fallan al correr. Se quedan en el
 *      panel, que es su fuente única.
 *   2. Los campos que son arrays (conditions, aggregations, operations, casts,
 *      windows...) necesitan su propio editor con altas y bajas. Meter eso en una
 *      tarjeta de 214px no ayuda a nadie.
 *
 * Lo que no está aquí sigue viviendo en el panel, al que se llega con el enlace
 * de "más ajustes" del propio nodo. `type` es 'text' o 'textarea'.
 */
export const INLINE_FIELDS = {
    sql_file:        [{ key: 'filePath', label: 'File', type: 'text' }],
    sql_inline:      [{ key: 'query', label: 'Query', type: 'textarea' }],
    import_file:     [{ key: 'sourcePath', label: 'File', type: 'text' },
                      { key: 'tableName', label: 'Table', type: 'text' }],
    import_folder:   [{ key: 'folderPath', label: 'Folder', type: 'text' },
                      { key: 'tableName', label: 'Table', type: 'text' }],
    export_file:     [{ key: 'outputPath', label: 'Output', type: 'text' }],
    create_table:    [{ key: 'tableName', label: 'Table', type: 'text' },
                      { key: 'query', label: 'Query', type: 'textarea' }],
    checkpoint:      [{ key: 'resumeLabel', label: 'Label', type: 'text' }],
    table_ref:       [{ key: 'tableName', label: 'Table', type: 'text' }],
    merge_tables:    [{ key: 'tableName', label: 'Table', type: 'text' }],
    assert:          [{ key: 'tableName', label: 'Table', type: 'text' }],
    join_tables:     [{ key: 'leftKey', label: 'Left key', type: 'text' },
                      { key: 'rightKey', label: 'Right key', type: 'text' }],
    filter:          [{ key: 'tableName', label: 'Table', type: 'text' }],
    group_aggregate: [{ key: 'tableName', label: 'Table', type: 'text' }],
    select_columns:  [{ key: 'tableName', label: 'Table', type: 'text' }],
    deduplicate:     [{ key: 'tableName', label: 'Table', type: 'text' }],
    add_column:      [{ key: 'tableName', label: 'Table', type: 'text' }],
    sort:            [{ key: 'tableName', label: 'Table', type: 'text' }],
    sample:          [{ key: 'sampleValue', label: 'Amount', type: 'text' }],
    pivot:           [{ key: 'groupColumn', label: 'Group by', type: 'text' },
                      { key: 'pivotColumn', label: 'Pivot on', type: 'text' },
                      { key: 'valueColumn', label: 'Values', type: 'text' }],
    rename_table:    [{ key: 'newName', label: 'New name', type: 'text' }],
    type_cast:       [{ key: 'tableName', label: 'Table', type: 'text' }],
    window_functions:[{ key: 'tableName', label: 'Table', type: 'text' }],
    unpivot:         [{ key: 'tableName', label: 'Table', type: 'text' }],
    bucket_read:     [{ key: 'uri', label: 'URI', type: 'text' },
                      { key: 'tableName', label: 'Table', type: 'text' }],
    gsheet_read:     [{ key: 'spreadsheetId', label: 'Spreadsheet', type: 'text' },
                      { key: 'sheet', label: 'Sheet', type: 'text' }],
    ai_enrich:       [{ key: 'inputColumn', label: 'Input column', type: 'text' },
                      { key: 'outputColumn', label: 'Output column', type: 'text' }],
    http_fetch:      [{ key: 'url', label: 'URL', type: 'text' },
                      { key: 'tableName', label: 'Table', type: 'text' }],
    clean:           [{ key: 'tableName', label: 'Table', type: 'text' }],
    date_ops:        [{ key: 'tableName', label: 'Table', type: 'text' }],
    flatten:         [{ key: 'column', label: 'Column', type: 'text' }],
    notification:    [{ key: 'message', label: 'Message', type: 'textarea' }],
    chart:           [{ key: 'chartTitle', label: 'Title', type: 'text' }],
    report:          [{ key: 'title', label: 'Title', type: 'text' }],
};
