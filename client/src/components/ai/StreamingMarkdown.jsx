...
export const MarkdownChunk = memo(({ content, components, urlTransform }) => (
    <ReactMarkdown remarkPlugins={[remarkGfm]} components={components} urlTransform={urlTransform}>{content}</ReactMarkdown>
));
 ...
 */
const StreamingMarkdown = ({ content, components, urlTransform }) => {
    ...
    return (
        <>
            {stable ? <MarkdownChunk content={stable} components={components} urlTransform={urlTransform} /> : null}
            {tail ? <ReactMarkdown remarkPlugins={[remarkGfm]} components={components} urlTransform={urlTransform}>{tail}</ReactMarkdown> : null}
        </>
    );
};
